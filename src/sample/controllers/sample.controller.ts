...
import { AppService, DatabaseService } from '../providers';
...
export class SampleController {
  constructor(
    @InjectPinoLogger(SampleController.name)
  ...
  public helloNumber(
    @Param('foo') foo: number,
    @Query('bar') bar: string,
    @Query('blah', ParseIntPipe) blah: number,
  ): AnyObject {
    return { foo, bar, blah };
  }
  ...
  @Get('samplegrpc') // http://localhost:3000/test/sample/samplegrpc
  public samplegrpc(): any {
